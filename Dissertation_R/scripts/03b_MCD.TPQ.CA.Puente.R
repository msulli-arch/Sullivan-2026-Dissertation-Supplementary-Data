# MCD/TPQ AND CA ANALYSIS FOR PUENTE (SA24) Original Catalog Record ####
# MCD-CA Code.R
...
#### 2. load  ceramic data ####
# Load original HSAPB catalog of ceramics from Puente to run analysis across site #
Puente_non_DAACS <- read_csv("data_raw/Puente/Puente_Non_DAACS_cerams.csv") %>%
...
PuenteContexts <- PhD_contexts %>%
  filter(`Project Name` == "Puente") %>%
  select(`Project Name`, `F S Number`, `FS Name`, FSGroup, `Unit`, `ASSOCIATED LEVEL`, `Deposit Type`)
# Connect Context data to original catalog record of Puente
Puente_cerams <- left_join(Puente_non_DAACS, PuenteContexts, by = c("F S Number"))

##### Translate Puente to DAACS prior to MCD, CA =====
...
# Show Puente Ware Types that are not represented in translation list
CheckWares <- anti_join(Wares, Ware, by= "true_code")

# Translate to DAACS Ware Types
SimpleTranslate <- Translate %>%
  select(-c("FLBeginDate", "FLEndDate", "StylisticGenre"))

Combined_data <- merge(Puente_cerams, SimpleTranslate,
                  by.x=c("Item"),
                  by.y=c("true_code")) %>%
  rename(QuadratID = "Unit")

# For MCD which includes Majolica and CEW Types defined for MCD and CA
# Define Majolicas present on site
Majolicas <- Combined_data %>%
...
# do a summary
summary1 <- Combined_data %>%
  group_by(`F S Number`, Ware) %>% 
  summarise(count = sum(Count))
summary1
...
DistinctUnits <- Combined_data %>%
  distinct(QuadratID)
DistinctUnits

# Consider Test Units
wareTypeData <- Combined_data %>%
  filter(!is.na(QuadratID))
FS_Units <- wareTypeData %>%
  distinct(QuadratID, `F S Number`)
CountUnits <- wareTypeData %>%
  distinct(QuadratID, `F S Number`) %>%
  count(QuadratID)
CountUnits

# we will keep all contexts when calculating the MCD, we may select out some units for CA analysis
...
wareTypeData_Unit <- wareTypeData %>%  
  mutate(unit = wareTypeData$`F S Number`)
...
MCDs1 <- merge(PuenteContexts, MCDs, by.x=c("F S Number"), by.y=c("unit")) %>%
  # Round the MCDs Columns to 2 decimal places
  mutate_if(is.numeric, ~ round(., 2))
write.csv(MCDs1,"data_processed/MCDs/Puente/MCDS.TPQS.CONTEXTS.Puente.csv")
...
MCDByUnit
MCDs <- as_tibble(MCDByUnit[["MCDs"]])
MCDs1 <- merge(PuenteContexts, MCDs, by.x=c("FSGroup"), by.y=c("unit")) %>%
  select(-c("F S Number", "FS Name", "ASSOCIATED LEVEL", "Unit", "Deposit Type")) %>%
  # Round the MCDs Columns to 2 decimal places
  mutate_if(is.numeric, ~ round(., 2)) %>%
  distinct()
write.csv(MCDs1,"data_processed/MCDs/Puente/MCDS.TPQS.FSGroup.Puente.csv")

# PUENTE CA WITH MAJOLICA AND CEW DIFFERENTIATION FROM ORIGINAL CATALOG DATA #####
wareTypeAndSizeData <- wareTypeData

#### 1.0  Do some clean up on Context and related data  ####
wareTypeAndSizeData1 <- wareTypeAndSizeData %>% 
  mutate(FSGroup = ifelse(is.na(FSGroup), '', FSGroup),
...
#### 3.0 Remove unid. types if you want, make Spanish Olive Jar a category ####
...
wareTypeAndSizeData2 <- wareTypeAndSizeData1 %>% 
  ...
  mutate(Ware = case_when(`CEW Type` %in% OliveJar ~ "Olive Jar",
                          TRUE ~ Ware))
...
wareTypeAndSize_Context <- wareTypeAndSizeData3 %>%
  mutate(StratigraphicZone = `F S Number`)
...
wareGenreCountsByContextT <- wareTypeAndSize_Context %>% 
  group_by(`F S Number`, StratigraphicZone, 
           Ware, `FSGroup`, `ASSOCIATED LEVEL`) %>% 
  summarize(Count= sum(Count)) %>% 
  pivot_wider(id_cols = c(`F S Number`, StratigraphicZone, FSGroup, `ASSOCIATED LEVEL`), 
...
p2 <- ggplot(data=means_and_cls, aes(x = Dimension, y=actual)) +
  ...
  labs( title="CA: Ware (Basic MAJ)",
        subtitle = 'PUENTE',
        x="Dimension", y='Proportion of Inertia')
p2

ggsave( p2, file = 'data_processed/CAs/Puente/CA_by_context/p2_Scree_Randomized_Inertias_PUENTE.png',  
...
colScores <- data.frame(ca1$colcoord[,1:4], 
                        type =ca1$colnames)
# fill in regional data for subdivided Spanish Wares
...
MCD_contexts <- read_csv("data_processed/MCDs/Puente/MCDS.TPQS.CONTEXTS.Puente.csv") %>%
  mutate(`F S Number` = as.character(`F S Number`)) %>%
  mutate(Selected = case_when(FSGroup == "F16" ~ FSGroup,
                              FSGroup == "TU8-Privy/Trash" ~ FSGroup,
                              FSGroup == "TU9-TrashPit/Privy?" ~ FSGroup,
                              TRUE ~ "Other Context")) %>%
  distinct(`F S Number`, blueMCD, TPQp95, Selected, FSGroup)

MCD_scaledrows <- left_join(scaledRowScores, MCD_contexts)
MCD_scaledrows <- left_join(MCD_scaledrows, FS_Units)

write.csv(scaledRowScores, "data_processed/CAs/Puente/CA_by_context/Scaled_Rows_CA_Puente.csv")
...
ggsave(p3, file = 'data_processed/CAs/Puente/CA_by_context/p3_BrokenStickScree_PUENTE_ware.png',  
       dpi=600, width=10, height=6, scale=1) 
...
set.seed(42)
p4 <- ggplot(MCD_scaledrows, aes(x=Dim1,y=Dim2,fill=Selected)) +
  coord_fixed() +
  geom_point(shape=21, size=3, colour="black", alpha=.75) +
  scale_fill_viridis_d() +
  geom_hline(yintercept = 0, linetype='dashed', color='grey') +
  geom_vline(xintercept = 0, linetype='dashed', color='grey') +
  labs(title = "Puente (SA24) Context Scores",
       x = paste ("Dimension 1",":  ", round(inertia[1,]*100),'%', sep=''), 
       y= paste ("Dimension 2",":  ", round(inertia[2,]*100),'%', sep='')) +
  theme_my()

p4
ggsave(p4, file = 'data_processed/CAs/Puente/CA_by_context/p4_ContextDim1Dim2_Puente_ware.png',  
       dpi=600) 

#### 10.4 Plot the row scores on CA Dim 1 and Dim3 #####
set.seed(42)
p5 <- ggplot(MCD_scaledrows, aes(x=Dim1,y=Dim3,fill=Selected)) +
  coord_fixed() +
  theme_my() +
  geom_point(shape=21, size=3, colour="black", alpha=.75) +
  scale_fill_viridis_d() +
...
p5
ggsave(p5, file = 'data_processed/CAs/Puente/CA_by_context/p5_ContextDim1Dim3_PUENTE_ware.png',  
       dpi=600, width = 6, height = 10, units = "in") 


# blue MCD and Dim1 as Plot ####
set.seed(42)
p4a <- ggplot(MCD_scaledrows, aes(x=Dim1,y=blueMCD,fill=Selected)) +
  theme_my() +
  geom_point(shape=21, size=3, colour="black", alpha=.75) +
  scale_fill_viridis_d() +
  geom_text_repel(aes(label=FSGroup), size = 3, cex= 4, force=2) +
  labs(title = 'Puente (SA24) MCD & CA (Dim1)',
       x = paste ("Dimension 1",":  ", round(inertia[1,]*100),'%', sep='')) 
p4a
ggsave(p4a, file = 'data_processed/CAs/Puente/CA_by_context/p4a_ContextMCDDim1_Puente_ware.png',  
       dpi=600)

# blue MCD and Dim2 as Plot
set.seed(42)
p4b <- ggplot(MCD_scaledrows, aes(x=Dim2,y=blueMCD,fill=Selected)) +
  theme_my() +
  geom_point(shape=21, size=3, colour="black", alpha=.75) +
  scale_fill_viridis_d() +
  geom_text_repel(aes(label=FSGroup), size = 3, cex= 4, force=2) +
  labs(title = 'Puente (SA24) MCD & CA (Dim2)',
       x = paste ("Dimension 2",":  ", round(inertia[2,]*100),'%', sep=''), 
       y= paste ("blueMCD"))

p4b
ggsave(p4b, file = 'data_processed/CAs/Puente/CA_by_context/p4b_ContextMCDDim2_Puente_ware.png',  
       dpi=600) 

# blue MCD and Dim3 as Plot
set.seed(42)
p4c <- ggplot(MCD_scaledrows, aes(x=Dim3,y=blueMCD,fill=Selected)) +
  theme_my() +
  geom_point(shape=21, size=3, colour="black", alpha=.75) +
  scale_fill_viridis_d() +
  geom_text_repel(aes(label=FSGroup), size = 3, cex= 4, force=2) +
  labs(title = 'Puente (SA24) MCD & CA (Dim3)',
       x = paste ("Dimension 3",":  ", round(inertia[3,]*100),'%', sep=''))

p4c
ggsave(p4c, file = 'data_processed/CAs/Puente/CA_by_context/p4c_ContextMCDDim3_Puente_ware.png',  
       dpi=600) 
...
p6 <- ggplot(colScores, aes(x = Dim1,y = Dim2,fill=General)) +
  ...
  geom_hline(yintercept = 0, linetype='dashed', color='grey') +
  geom_vline(xintercept = 0, linetype='dashed', color='grey') +
  labs(title = 'Puente (SA24) Ware Type Scores (Dim1 & 2)',
       x = paste ("Dimension 1",":  ", round(inertia[1,]*100),'%', sep=''), 
       y= paste ("Dimension 2",":  ", round(inertia[2,]*100),'%', sep=''))
  
p6
ggsave(p6, file = 'data_processed/CAs/Puente/CA_by_context/p6_TypeDim1Dim2_PUENTE_ware.png',  
       dpi=600, width = 7, height = 10, units = "in") 
...
p7 <- ggplot(colScores, aes(x = Dim1,y = Dim3, fill=General)) +
  ...
  geom_hline(yintercept = 0, linetype='dashed', color='grey') +
  geom_vline(xintercept = 0, linetype='dashed', color='grey') +
  labs(title = 'Puente (SA24) Ware Type Scores (Dim1 & 3)',
       x = paste ("Dimension 1",":  ", round(inertia[1,]*100),'%', sep=''), 
       y= paste ("Dimension 3",":  ", round(inertia[3,]*100),'%', sep='')) +
  theme_my()  
p7
ggsave(p7, file = 'data_processed/CAs/Puente/CA_by_context/p7_TypeDim1Dim3_PUENTE_ware.png',  
       dpi=600, width = 6, height = 10, units = "in")

# PUENTE FSGROUP CA ANALYSIS WITH MAJOLICA AND CEW DIFFERENTIATION FROM ORIGINAL CATALOG DATA #####

wareGenreCountsByContextT <- wareTypeAndSize_FSGroup %>% 
  group_by(StratigraphicZone, 
           Ware) %>% 
  summarize(Count= sum(Count)) %>% 
  pivot_wider(id_cols = c(StratigraphicZone), 
              names_from = Ware, values_from= Count, values_fill = 0) %>%
  filter (StratigraphicZone != "")

ContextCheck <- distinct(wareGenreCountsByContextT, StratigraphicZone)
print(ContextCheck, n=250)

##### 9.1 CA Stats and Plots #####
...
ggsave( p2, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p2_Scree_Randomized_Inertias_PUENTE.png',  
        dpi=600, width=10, height=6, scale=1) 
...
# Load Context MCDs and add them to Scaled Rows for future analysis
MCD_contexts <- read_csv("data_processed/MCDs/Puente/MCDS.TPQS.FSGroup.Puente.csv") %>%
  mutate(Selected = case_when(FSGroup == "F16" ~ FSGroup,
                              FSGroup == "TU8-Privy/Trash" ~ FSGroup,
                              FSGroup == "TU9-TrashPit/Privy?" ~ FSGroup,
                              TRUE ~ "Other Context")) %>%
  distinct(blueMCD, TPQp95, Selected, FSGroup)

MCD_scaledrows <- left_join(scaledRowScores, MCD_contexts, by = c("unit" = "FSGroup"))
write.csv(scaledRowScores, "data_processed/CAs/Puente/CA_by_FSGroup/PUENTE_Scaled_Rows_CA.csv")
...
ggsave(p3, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p3_BrokenStickScree_PUENTE.png',  
       dpi=600, width=10, height=6, scale=1) 
...
p4 <- ggplot(MCD_scaledrows, aes(x=Dim1,y=Dim2,fill=Selected)) +
  ...
  geom_hline(yintercept = 0, linetype='dashed', color='grey') +
  geom_vline(xintercept = 0, linetype='dashed', color='grey') +
  geom_text_repel(aes(label= unit), size = 3, cex = 4, max.overlaps=20) +
  labs(title = "Puente (SA24) Context Scores",
       x = paste ("Dimension 1",":  ", round(inertia[1,]*100),'%', sep=''), 
       y= paste ("Dimension 2",":  ", round(inertia[2,]*100),'%', sep='')) 

p4
ggsave(p4, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p4_ContextDim1Dim2_Puente.png',  
       dpi=600, width=12, height=12, scale=1) 
...
p5 <- ggplot(MCD_scaledrows, aes(x=Dim1,y=Dim3,fill=Selected)) +
  ...
  labs(title = "Puente (SA24) Context Scores",
       ...
       y= paste ("Dimension 3",":  ", round(inertia[3,]*100),'%', sep='')) 
print(p5)
ggsave(p5, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p5_ContextDim1Dim3_PUENTE_ware.png',  
...
p4a <- ggplot(MCD_scaledrows, aes(x=Dim1,y=blueMCD,fill=Selected)) +
  theme_my() +
  geom_point(shape=21, size=4, colour="black", alpha=.75) +
  scale_fill_viridis_d() +
  geom_text_repel(aes(label= unit), size = 3, cex = 4 ,max.overlaps=20) +
  labs(title = "Puente (SA24) Dim 1 and blueMCD",
       x = paste ("Dimension 1",":  ", round(inertia[1,]*100),'%', sep=''))
p4a
ggsave(p4a, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p4a_ContextMCDDim1_Puente_ware.png',  
       dpi=600, width=12, height=6, scale=1) 
...
p4b <- ggplot(MCD_scaledrows, aes(x=Dim2,y=blueMCD,fill=Selected)) +
  ...
  labs(title = "Puente (SA24) Dim 2 and blueMCD",
       x = paste ("Dimension 2",":  ", round(inertia[2,]*100),'%', sep=''), 
       y= paste ("blueMCD"))

p4b
ggsave(p4b, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p4b_ContextMCDDim2_Puente_ware.png',  
       dpi=600, width=12, height=6, scale=1) 

set.seed(42)
p4c <- ggplot(MCD_scaledrows, aes(x=Dim3,y=blueMCD,fill=Selected)) +
  theme_my() +
  geom_point(shape=21, size=5, colour="black", alpha=.75) +
  scale_fill_viridis_d() +
  geom_text_repel(aes(label=unit), size = 3, cex= 4, force=2) +
  labs(title = "Puente (SA24) Dim 3 and blueMCD",
       x = paste ("Dimension 3",":  ", round(inertia[3,]*100),'%', sep=''))

p4c
ggsave(p4c, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p4c_ContextMCDDim3_Puente_ware.png',  
       dpi=600, width=12, height=6, scale=1)
...
p6 <- ggplot(colScores, aes(x = Dim1,y = Dim2,fill=General)) +
  ...
  geom_hline(yintercept = 0, linetype='dashed', color='grey') +
  geom_vline(xintercept = 0, linetype='dashed', color='grey') +
  labs(title = 'Puente (SA24) Ware Type Scores',
       x = paste ("Dimension 1",":  ", round(inertia[1,]*100),'%', sep=''), 
       y= paste ("Dimension 2",":  ", round(inertia[2,]*100),'%', sep=''))  
p6
ggsave(p6, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p6_TypeDim1Dim2_PUENTE_ware.png',  
       dpi=600, width=12, height=12, scale=1)
...
p7 <- ggplot(colScores, aes(x = Dim1,y = Dim3, fill=General)) +
  ...
  labs(title = 'Puente (SA24) Ware Type Scores',
       ...
       y= paste ("Dimension 3",":  ", round(inertia[3,]*100),'%', sep=''))  
p7
ggsave(p7, file = 'data_processed/CAs/Puente/CA_by_FSGroup/p7_TypeDim1Dim3_PUENTE_ware.png',  
       dpi=600, width=12, height=12, scale=1)
